          ...
          return this.personSvc.getByDocument(doc).pipe(
            ...
            catchError((err) => {
              if (err?.status === 404) {
                ...
                this.sweetAlertService.showNotification('No encontrado', msg, 'warning');
      ...
      .subscribe((person: any | null) => {
        ...
        if (person) {
          ...
          if (currentDoc && currentDoc.length >= 5 && this.lastNotFoundAlertForDoc !== currentDoc) {
            this.sweetAlertService.showNotification('No encontrado', 'No existe una persona con el documento ingresado.', 'warning');
          }
          this.resetFoundPerson();
        }
      });

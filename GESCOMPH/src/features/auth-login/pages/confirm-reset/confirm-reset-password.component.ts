...
export class ConfirmResetPasswordComponent {
  ...
  async submit() {
    if (this.form.invalid) {
      ...
      this.sweetAlertService.showNotification('Advertencia', 'Verifica los campos del formulario.', 'warning');
      return;
    }
    ...
    this.sweetAlertService.showLoading('Confirmando', 'Por favor, espera...');
    this.auth.ConfirmPasswordReset(dto).subscribe({
      next: async () => {
        await this.sweetAlertService.hideLoading();
        await this.sweetAlertService.success('Contraseña actualizada correctamente.');

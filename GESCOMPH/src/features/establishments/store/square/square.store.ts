...
export class SquareStore {
  ...
  async changeActiveStatusRemote(id: number, active: boolean): Promise<{ ok: boolean; message?: string }> {
    if (this.isBusy(id)) return { ok: false, message: 'Operaci�n en curso' };
    ...
    try {
      ...
      // Envelope controlado desde backend (200 OK)
      if (updated && typeof updated === 'object' && (updated as any).success === false) {
        if (prev !== undefined) this.changeActiveStatus(id, prev);
        return { ok: false, message: (updated as any).message || 'No se pudo cambiar el estado.' };
      }

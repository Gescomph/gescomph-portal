...
import { FormGroup, FormBuilder, FormControl, ValidatorFn, Validators, AbstractControl, ReactiveFormsModule } from '@angular/forms';
...
import { FormType, formSchemas } from './generic-form.config';
...
export class GenericFormComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    const selectChanges = changes['selectOptions'];
    if (selectChanges) {
      this.applySelectOptions(selectChanges.currentValue);
    }
  }

  private applySelectOptions(options?: Record<string, Option[]>): void {
    if (!options || !this.fields?.length) return;
    this.fields.forEach(field => {
      if (Object.prototype.hasOwnProperty.call(options, field.name)) {
        this.setOptionsForField(field.name, options[field.name] ?? []);
      }
    });
  }
  ...
  ngOnInit() {
    ...
    if (this.hiddenFields?.length) {
      this.fields.forEach(field => {
        if (this.hiddenFields.includes(field.name)) {
          field.type = 'hidden';
    ...
    this.applySelectOptions(this.selectOptions);
    ...
    // Validación cruzada específica para SystemParameter: effectiveTo >= effectiveFrom
    if (this.formType === 'SystemParameter') {
  ...
  onTrim(name: string) {
    ...
    if (typeof v === 'string') {
      ...
      let next = t;
      // Normaliza 'key' en SystemParameter a MAYÚSCULAS para evitar rechazos del backend
      if (this.formType === 'SystemParameter' && name === 'key') {
  ...
  onSubmit() {
    ...
    for (const field of this.fields as any[]) {
      if (field.type === 'checkbox-list') {
        ...
        // Backend de SystemParameter.Value es string: enviar como string en SystemParameter
        if (this.formType === 'SystemParameter' && field.name === 'value') {
